...
function article_getByEncodedName(code_name: string) {
  return pages[code_name] || pages[encodeURIComponent(code_name)]
}

function article_create(name: string, contents: string) {
  if (pages[name]) {
...
function article_deleteByEncodedName(code_name: string) {
  if (pages[code_name]) {
...
function article_editByEncodedName(code_name: string, contents: string) {
  if (pages[code_name]) {
...
function article_searchByTerms(searchterms: string) {
...
function article_getRandoms(n: number) {
  ...
  return Array.from(results_index_set).map(i => pages_array[i as number])
}
